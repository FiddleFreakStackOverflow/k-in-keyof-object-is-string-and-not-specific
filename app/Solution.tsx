...
type Screens = Partial<{
  [K in keyof StackParamsList]: {
    screen: React.ComponentType<any>;
    screenTitle: string;
    stackGroupType: StackGroupType;
  };
}>;
...
function getScreensWithoutHome(Stack: NavigationStack, defaultHome: ScreenSelection): React.JSX.Element[] {
  const screensWithoutHome: Screens = Object.fromEntries(
    Object.entries(screens).filter(([key, value]) => key !== defaultHome.route)
  );
  return Object.entries(screensWithoutHome).map(([key, value]) => (
    <Stack.Screen
...
export function GroupMain(Stack: NavigationStack): ReactNode {
  return (
    <Stack.Group>
      <Stack.Screen
      ...
      />
      {getScreensWithoutHome(Stack, defaultHome)}
    </Stack.Group>
  );
}
